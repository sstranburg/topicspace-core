...
**The architecture's value proposition decomposes into three consequences of the same root property:** smaller planner inputs, better planning on measured operational tasks, and a human-inspectable state substrate. The first two are measured in the experiments reported here. The third is an architectural property of the asymmetric-projection design — the substrate preserves warrants and lifecycle for human inspection while the planner consumes sparse names — and is reported as design-time, not as a measured governance outcome.

...
2. **F-007 V2 phase 2 — inverted-region bias investigation + escalation routing.** Phase 1 shipped 2026-05-21 (rolling primitive, region rolling walk-forward, persistence rolling re-test, inverted-flag artifact). Phase 2: investigate the sign=−1 dominance in the inverted list (7 of 10 inverted public regions are bearish reads where price did the opposite — likely L2-direction-extractor bias on bearish themes), then design the escalation destination (alert table? Slack route? review queue?). Conviction-reweighting (the V2-acts step) gated on this.
...
- **followups_for_v2**:
  ...
  - Re-weight L2 conviction by region calibration (conviction → region-conditioned probability). **V2 phase 2 deferred.** Gated on (a) inverted-region bias investigation finishing, (b) at least one more month of data to expand each fold's per-region n.

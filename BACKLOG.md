...
1. **F-007 V2 — region-level walk-forward (rolling) + inverted-region detection.** V1 (2026-05-18) confirmed regional heterogeneity is real but is in-sample only. V2 adds rolling walk-forward per region (once corpus supports stable folds), an escalation rule for chronically inverted regions (hit ≤ 30% on n≥10 → upstream review), and re-weighting of L2 conviction by region calibration. Now also doubles as the rigorous re-test of F-006 #6 (persistence as prior weight) — see F-006 followup notes.
...
3. **F-006 V2 — per-expectation walk-forward + reconfirmed event type (DONE 2026-05-21).** V1 measurement script `scripts/run_lifecycle_walkforward.py` ships; #5 (reconfirmed event type) folded into `build_expectation_lifecycle.py`. Single-split finding: persistence shows positive train+test spread at 5d / 10d / 20d horizons after the reconfirmed fix, but only marginally and on small samples. Verdict: rolling walk-forward (subsumed by F-007 V2) before any L1 prior-weight build (F-006 #6).

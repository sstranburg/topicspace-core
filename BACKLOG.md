...
2. **F-007 V2 phase 2 — sign-flip rule + label-staleness fix.** Phase 1 shipped + bias investigation completed 2026-05-21 (see `data/derived/inverted_regions_investigation.md`). Verdict: 9 of 10 inverted public regions fit the **fade dynamic** hypothesis — the L2 extractor reads analyst notes correctly bearish/bullish, but strong consensus on hot AI themes is itself a contrarian 5d signal. Not an extractor bug. The V2-acts step is therefore (a) build a per-region sign-flip rule (treat L2 direction as inverted when corpus hit_5d ≤ 30% AND rolling range ≤ 20pp), and (b) fix cluster-label staleness (the one mislabel found: "Datadog's strong Q4" attached to bearish DDOG selloff reads). Defer human-routing escalation — automatic re-direction is the higher-leverage move.

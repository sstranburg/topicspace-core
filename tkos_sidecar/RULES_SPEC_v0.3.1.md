# Belief Derivation Spec — RULES_SPEC_v0.3.1

**Date:** 2026-06-06 (v0.2 → v0.2.1 → v0.3.1, this version)
**Status:** LOCKED at v0.3.1. This is the lock-before-code artifact for the TKOS write-path build.
...
TRIGGERS_ON: scan at the start of each new turn
PRECONDITIONS:
  - There exists a tool_call event E whose turn_idx is T
  - No tool_result with parent_event_id == E.source_event_id has been observed at any event in turns T, T+1, T+2 (i.e., within K=3 subsequent TURNS, not events)
  - The current turn_idx is ≥ T+3
...
OBSERVED_AT_TURN: current turn_idx  (T+3 or later — when the retro-mint condition first becomes true)
AUTHORITY: asserted_by_assistant

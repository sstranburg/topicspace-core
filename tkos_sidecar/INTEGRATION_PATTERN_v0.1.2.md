# Belief Stack Integration Pattern v0.1.2

**Date:** 2026-06-06 (v0.1 → v0.1.1 → v0.1.2, this version)
**Status:** v0.1.2 working draft — harness-focused, not product copy. Iterates as integrations land.
...
Recognized header markers (locked):
- `*** Add File: <path>` — file being created
- `*** Update File: <path>` — file being modified
- `*** Delete File: <path>` — file being removed
- `*** Move File: <from> -> <to>` — file being renamed (returns both `from` and `to`)

```python
import re
PATCH_HEADER_RE = re.compile(
    r"^\*\*\* (Add|Update|Delete|Move) File:\s*([^\s]+)(?:\s*->\s*([^\s]+))?",
    re.MULTILINE
)

def extract_paths_from_patch(patch_text: str) -> list[str]:
    paths: list[str] = []
    for m in PATCH_HEADER_RE.finditer(patch_text):
        paths.append(m.group(2))
        if m.group(3):  # Move File: a -> b

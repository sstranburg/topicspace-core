# Belief Derivation Spec — RULES_SPEC_v0.3.2

**Date:** 2026-06-06 (v0.2 → v0.2.1 → v0.3.1 → v0.3.2, this version)
**Status:** LOCKED at v0.3.2. This is the lock-before-code artifact for the TKOS write-path build.

**v0.3.1 → v0.3.2 patches (two fixes in this doc):**
- **Fix IV — `pipeline_running_born_retroactive` dispatch path.** v0.3.1 said the rule "triggers at the start of each new turn," but the rule engine dispatches by `event_type`. v0.3.2 §3.4 specifies the trigger concretely: when `ingest_source_line()` processes a mapped event whose `turn_idx` is strictly greater than the previous max for the session, the engine runs a retroactive scan over all unmatched `tool_call`s in turns ≤ `(new_turn_idx - 3)`. The scan is part of the same atomic transaction.
- **Fix V — `report_ready` depends on successful result.** v0.3.1 §3.7 `report_ready_born` fired on the `apply_patch` (or write_file) `tool_call`. A malformed or rejected patch would still mint the belief. v0.3.2 §3.7 changes the trigger to the `tool_result` event with `exit_code = 0` AND the parent `tool_call`'s paths matching `REPORT_PATH_PATTERNS`.

**v0.2.1 → v0.3.1 patch (one fix in this doc):**
- **Fix 4 — K=3 retro-mint semantic.** §3.4 `pipeline_running_born_retroactive` previously waited for K=3 subsequent *events*. Real Codex sessions emit several parallel `tool_call`s before their `tool_result`s arrive, often within the same conversational turn. The K=3 rule would falsely fire on every `tool_call` in a parallel batch. Locked semantic: **K=3 means three subsequent TURNS, not events.** This restores `effective_turn < observed_at_turn` as the strict retro-mint signature, and avoids false-fires during normal Codex parallel tool dispatch.
...
TRIGGERS_ON: ingest_source_line() processing a MAPPED event whose turn_idx N is strictly greater than the prior maximum turn_idx for the session (i.e., the turn boundary just advanced).
DISPATCH: the engine runs this scan as part of the same atomic transaction as the mapped-event ingest, BEFORE the event's own event-type rules fire.
PRECONDITIONS (per candidate tool_call):
  - There exists a tool_call event E whose turn_idx T satisfies T ≤ N − 3
  - No tool_result with parent_event_id == E.source_event_id exists in events with turn_idx ≤ N − 1
  - No pipeline_running belief already exists referencing E.source_event_id
ACTION (for each candidate E that passes preconditions):
  - Mint pipeline_running referencing E
EFFECTIVE_TURN: T   (the original tool_call's turn)
OBSERVED_AT_TURN: N (the new turn that triggered the scan; T+3 ≤ N)
AUTHORITY: asserted_by_assistant
NOTE_TEMPLATE: "pipeline_running — {tool_name} {command} from turn {effective_turn} (observed at turn {observed_at_turn})"
```

**Dispatch path (v0.3.2 — fix IV):** the `pipeline_running` scan is NOT dispatched by an event_type. It is dispatched by the *turn-boundary advance condition*. When `ingest_source_line()` (per TKOS_WRITE_PATH_SCOPE §6.1) processes a mapped event whose `turn_idx` is the first mapped event of a new turn for the session, the engine executes:

```
SELECT E.* FROM events E
LEFT JOIN events R
  ON R.event_type = 'tool_result'
  AND R.parent_event_id = E.source_event_id
  AND R.turn <= N - 1
...
For each row returned, the engine writes a `pipeline_running` belief_event with `effective_turn = E.turn` and `observed_at_turn = N`. All of this commits in the same transaction as the triggering event's ingest. If any rule throws, the transaction rolls back per §6.1.

**K=3 unit (v0.3.1 — fix 4):** *turns*, not events. Real agent harnesses (Codex, Claude Code, etc.) commonly emit multiple parallel `tool_call`s within a single turn. Counting subsequent events would falsely fire the rule on every parallel call before its matching `tool_result` arrives. Counting subsequent turns correctly identifies a long-running tool call that has not produced a result across the next three planning turns — the operational signal the spec actually intends.

Note: `effective_turn` strictly less than `observed_at_turn` remains the retro-mint signature. By the K=3-turns rule, `observed_at_turn ≥ effective_turn + 3`. The read-path returns `pipeline_running` for any `state(turn=Q)` where `effective_turn ≤ Q`, even when `observed_at_turn > Q`.
...
TRIGGERS_ON: tool_result WHERE event.exit_code == 0   (v0.3.2 — fix V: was tool_call)
PRECONDITIONS:
  - The parent tool_call (looked up by parent_event_id) has tool_name in
    {"write_file", "edit_file", "apply_patch"}
  - Any path in the parent tool_call's paths matches REPORT_PATH_PATTERNS (§6)
ACTION:
  - Mint report_ready with claim referencing the matched path
EFFECTIVE_TURN: event.turn_idx     (the tool_result's turn — when the write succeeded)
OBSERVED_AT_TURN: event.turn_idx
AUTHORITY: confirmed_by_tool       (was asserted_by_assistant — fix V: tool result is confirmation)
NOTE_TEMPLATE: "report_ready — {path} produced at turn {turn_idx} (exit 0)"
```

**Fix V rationale:** the prior v0.3.1 rule fired on the `tool_call` itself (the *attempt* to write the report), which meant a malformed `apply_patch`, a rejected `write_file` (permission error, disk full, etc.), or any other failure mode would still mint `report_ready`. v0.3.2 ties the belief to the successful `tool_result` — the report is only "ready" when the write actually succeeded. Authority upgraded from `asserted_by_assistant` to `confirmed_by_tool` accordingly.

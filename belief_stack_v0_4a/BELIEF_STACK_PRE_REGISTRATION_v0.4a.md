...
**Date:** 2026-06-03
**Status:** **LOCKED 2026-06-03.** All six decisions resolved; pre-registration is now closed. No amendments without re-lock and re-version (v0.4a.1, etc.).
**Lineage:** OB-001 (v0.1) → OB-002 (v0.2.2) → Belief Stack v0.3 (planning-side, locked + run 2026-06-03) → **v0.4a (mechanism ablation, this document)** → v0.4b (end-to-end cost) → v0.4c (replication)
...
| **D4** | Token budget for arms B/C/D/E | **RESOLVED** | All four arms targeted at ~285 input tokens ± 10% (matched to v0.3 Arm B budget). Budget cap enforced by §3.5a dedup-ranking with header reserve computed against worst-case render. **Locked: the experiment answers *what information shape matters?* — not *what budget is optimal?* Floating budgets would forfeit the interpretability that justifies running this at all.** |
...
- [x] D4 — matched token budget ~285 ± 10%
...
- [x] D8 — interpretation rules locked with Outcome 5 wording tightened per Sue's amendment
...
After lock: no amendments without a re-lock and re-version (v0.4a.1, etc.).
